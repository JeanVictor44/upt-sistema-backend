...
import { CreateTeachingPlaceUseCase } from '@root/domain/location/applications/use-cases/teaching-place/create-teaching-place.use-case'
import {
  CreateTeachingPlaceBodySwaggerDto,
  CreateTeachingPlaceResponseSwaggerDto,
  CreateTeachingPlaceSwaggerDto,
} from '@root/presentation/swagger/location/docs/teaching-place/create-teaching-place-swagger.dto'
...
export class CreateTeachingPlaceController {
  ...
  async handle(@Body() body: CreateTeachingPlaceBodySwaggerDto): Promise<CreateTeachingPlaceResponseSwaggerDto> {
    ...
    const result = await this.createTeachingPlace.execute({
      name,
      neighborhoodId,
