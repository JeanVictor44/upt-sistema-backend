...
import { EditTeachingPlaceUseCase } from '@root/domain/location/applications/use-cases/edit-teaching-place.use-case'
import {
  EditTeachingPlaceBodySwaggerDto,
  EditTeachingPlaceResponseSwaggerDto,
  EditTeachingPlaceSwaggerDto,
} from '@root/presentation/swagger/location/docs/edit-teaching-place-swagger.dto'
...
export class EditTeachingPlaceController {
  ...
  async handle(
    ...
    const result = await this.editTeachingPlace.execute({ name, neighborhoodId, id })
